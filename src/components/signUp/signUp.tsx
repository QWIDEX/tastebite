...
const SignUp = () => {
  ...
  const handleRegister: SubmitHandler<any> = async (data: FormInputs) => {
    if (!data.password) {
      setError(
        "password",
    ...
    try {
      ...
      if (reg.ok) {
        ...
        if (sign?.ok) toast.success("Sucessfully registered");
        else {
          console.log(sign?.error);
          toast.error("Something went wrong");
        }
      } else {
        const error = await reg.json();
  ...
  return (
    <div className="w-full mx-auto lg:w-[61%] flex items-center">
      <div className="gap-7 border border-gray-200 shadow-lg flex flex-col justify-center py-10 px-5 max-w-md w-full mx-auto rounded-lg ">
        <div className="mb-5">
          <H1 className="mb-5 text-center !font-sans">Sign Up</H1>
